...
- **Scrapling Evaluation**: Conducted an evaluation on the Python web scraping library `Scrapling` (https://github.com/D4Vinci/Scrapling) for its viability in extracting text for the project. Conclusion: It is not recommended to integrate at this time due to ecosystem clash (we use Node.js `PlaywrightCrawler`), environment dependency issues, and the fact that 403 blocks are better bypassed by headless browsers. Full details in `docs/scrapling_evaluation.md`.
