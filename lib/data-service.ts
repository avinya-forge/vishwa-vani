  ...
  private async enrichVerses(verses: unknown[], includeAI: boolean = false): Promise<EnrichedVerse[]> {
    return verses.map(verse => {
      ...
      const enriched: EnrichedVerse = {
        ...
        translation: (v.translation || v.meaning || (((v.layers as unknown[]) || []).find((l: unknown) => (l as Record<string, unknown>).type === 'translation') as Record<string, unknown>)?.content) as string | undefined,
