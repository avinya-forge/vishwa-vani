...
export default function header() {
  ...
  return (
    <nav className="w-full flex justify-center border-b border-stone-200 h-12 bg-white/70 backdrop-blur-xl sticky top-0 z-50 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)]">
      <div className="w-full max-wide flex justify-between items-center px-4 sm:px-6">
        
        <div className="flex gap-5 items-center">
          <Link href="/" className="text-lg font-serif font-black tracking-tighter text-orange-600 hover:scale-105 transition-transform">
            Vishwa-Vani
          </Link>
          
          <div className="hidden lg:flex items-center gap-5 text-[12px] font-semibold text-stone-500">
            <div className="relative group" 
                 onMouseEnter={() => setShowTopics(true)}
                 onMouseLeave={() => setShowTopics(false)}
            >
                <button 
                  className={`flex items-center gap-1.5 transition-colors uppercase tracking-widest text-[11px] ${showTopics ? 'text-orange-600' : 'hover:text-orange-600'}`}
                >
                 {t('library')}
                 <svg className={`w-3 h-3 transition-transform ${showTopics ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
               </button>
               
               {showTopics && (
                 <div className="absolute top-8 left-0 w-64 bg-white border border-stone-100 rounded-2xl shadow-2xl p-4 animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            <Link href="/search" className="hover:text-orange-600 transition-colors flex items-center gap-1.5 uppercase tracking-widest text-[10px]">
              {t('search')}
            </Link>

            <Link href="/lab" className="hover:text-orange-600 transition-colors flex items-center gap-1.5 uppercase tracking-widest text-[10px]">
              Labs
            </Link>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button 
              ...
              className="px-2.5 py-1 bg-stone-100 rounded-full text-[10px] font-bold text-stone-600 hover:bg-stone-200 transition-colors flex items-center gap-2"
            >
          ...
          <Link
            ...
            className="hidden sm:inline-flex py-1 px-4 rounded-full bg-stone-900 hover:bg-orange-600 text-white font-bold text-[11px] uppercase tracking-wider transition-all shadow-md active:scale-95"
          >
            {t('readNow') || 'Read Now'}
          </Link>
