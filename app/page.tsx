      ...
      <div className="max-wide mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 lg:px-4">
          {VEDIC_LIBRARY.map((text) => (
            <div 
              key={text.slug} 
              className={`group relative p-6 rounded-[2rem] border transition-all duration-700 flex flex-col h-full bg-white ${text.available ? 'border-stone-100 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.03)] hover:shadow-[0_25px_80px_-25px_rgba(0,0,0,0.1)] hover:-translate-y-2' : 'border-stone-50 opacity-60 bg-stone-50/50 grayscale'}`}
            >
